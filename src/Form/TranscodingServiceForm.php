...
class TranscodingServiceForm extends EntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        drupal_set_message($this->t('Created the %label Transcoding service.', [
          '%label' => $transcoding_service->label(),
        ]));
        break;

      default:
        drupal_set_message($this->t('Saved the %label Transcoding service.', [
          '%label' => $transcoding_service->label(),
        ]));
    }
    $form_state->setRedirectUrl($transcoding_service->urlInfo('collection'));
  }
